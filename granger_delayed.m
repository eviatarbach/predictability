    ...
    for lon = 1:480
        for lat = 1:241
            ...
            for delay = delays_i
                ...
                sst_ts_effect = sst_ts(delay:end);
                sst_ts_cause = sst_ts(1:end-delay);

                vort_ts_effect = vort_ts(delay:end);
                ...
                F_vort_to_sst_i{j}(lon, lat) = F_1;
                F_sst_to_vort_i{j}(lon, lat) = F_2;
                ...
                for alpha = [0.1, 0.05]
                    ...
                    if alpha == 0.1
                        sig_90_vort_to_sst_i{j}(lon, lat) = sig_vort_cause;
                        sig_90_sst_to_vort_i{j}(lon, lat) = sig_sst_cause;
                    else
                        sig_95_vort_to_sst_i{j}(lon, lat) = sig_vort_cause;
                        sig_95_sst_to_vort_i{j}(lon, lat) = sig_sst_cause;
                    end
                end
                j = j + 1;
            end
